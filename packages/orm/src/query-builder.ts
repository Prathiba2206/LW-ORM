...
export type Where<Row> = Partial<{ [K in keyof Row]: Row[K] }>;
...
function whereClause<Row>(where: Where<Row> | undefined, start = 1): SqlQuery {
  if (!where) return { text: "", values: [] };
  const entries = Object.entries(where).filter(([, value]) => value !== undefined);
  if (!entries.length) return { text: "", values: [] };
...
export function buildInsert(model: Model<string, Shape>, data: Record<string, unknown>): SqlQuery {
  const entries = Object.entries(data).filter(([, value]) => value !== undefined);
  if (!entries.length) return { text: `INSERT INTO ${quote(model.tableName)} DEFAULT VALUES RETURNING *`, values: [] };
  return {
...
export function buildSelect<M extends Model<string, Shape>>(model: M, args: { where?: Where<InferRow<M>>; orderBy?: OrderBy<InferRow<M>>; limit?: number; offset?: number } = {}): SqlQuery {
  const where = whereClause<InferRow<M>>(args.where);
  let text = `SELECT * FROM ${quote(model.tableName)}${where.text}`;
  const order = Object.entries(args.orderBy ?? {});
  if (order.length) text += ` ORDER BY ${order.map(([key, direction]) => `${quote(key)} ${String(direction).toUpperCase()}`).join(", ")}`;
  if (args.limit !== undefined) text += ` LIMIT ${Math.max(0, Math.trunc(args.limit))}`;
  if (args.offset !== undefined) text += ` OFFSET ${Math.max(0, Math.trunc(args.offset))}`;
...
export function buildUpdate<M extends Model<string, Shape>>(model: M, data: Partial<InferRow<M>>, whereInput: Where<InferRow<M>>): SqlQuery {
  const entries = Object.entries(data).filter(([, value]) => value !== undefined);
  if (!entries.length) throw new Error("Update data cannot be empty");
  const where = whereClause<InferRow<M>>(whereInput, entries.length + 1);
  if (!where.text) throw new Error("Update requires a non-empty where clause");
...
export function buildDelete<M extends Model<string, Shape>>(model: M, whereInput: Where<InferRow<M>>): SqlQuery {
  const where = whereClause<InferRow<M>>(whereInput);
  if (!where.text) throw new Error("Delete requires a non-empty where clause");
